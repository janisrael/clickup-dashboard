...
window.dashboardV2 = {
  ...
  createTimelineChart(data) {
    ...
    members.forEach((member, memberIndex) => {
      ...
      if (memberData.in_progress_periods) {
        // const period = memberData.in_progress_periods[0];
        // if (period) {
        ...
        //   const yPosition = memberIndex + -0.;
        //   //   const yPosition = memberIndex + 0 * 0.2 - 0.1; // Only index 0
      ...
      if (memberData.downtime_periods) {
        memberData.downtime_periods.forEach((period, downtimeIndex) => {
          ...
          const activeTaskCount = memberData.in_progress_periods
            ...
            : 0;
          const yPosition = memberIndex + (activeTaskCount + downtimeIndex) * 0.3;
        ...
        // const period = memberData.downtime_periods[0];
        // if (period) {
        //   const startTime = new Date(period.start);

        //   // Set endTime as 5:00 PM today
        //   const now = new Date();
        ...
        //   //   const yPosition = memberIndex + activeTaskCount * 0.2 + 0.1;
        //   const yPosition = memberIndex + activeTaskCount;
        //   datasets.push({
        //     label: `${member} - Downtime (${duration.toFixed(1)}h)`,
        //     data: [
        //       {
  ...
  //     // Process each member - keep bars aligned to member index
  //     members.forEach((member, memberIndex) => {
  //       const memberData = detailedData[member];

  //       // Active periods (green bars) - keep at exact member position
  //       if (memberData.in_progress_periods && memberData.in_progress_periods.length > 0) {
  //         // Combine all active periods for this member into one or separate datasets
  //         memberData.in_progress_periods.forEach((period, taskIndex) => {
  //           const startTime = new Date(period.start);
  //           const now = new Date();
  //           const endTime = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 17, 0, 0, 0);

  //           // Keep y position exactly at memberIndex for proper alignment
  //           datasets.push({
  //             label: `${member} - ${period.task_name}`,
  //             data: [
  //               {
  //                 x: [startTime, endTime],
  //                 y: memberIndex, // No offset - keep exactly at member position
  //               },
  //             ],
  //             backgroundColor: this.colors.active,
  //             borderColor: "darkgreen",
  //             borderWidth: 1,
  //             barThickness: 18, // Slightly thinner to allow for multiple tasks
  //             categoryPercentage: 0.9,
  //             type: "bar",
  //             order: 1,
  //           });
  ...
  //       // Downtime periods (red/yellow bars) - also keep at exact member position
  ...
  //           // Keep y position exactly at memberIndex
  //           datasets.push({
  //             label: `${member} - Downtime (${duration.toFixed(1)}h)`,
  //             data: [
  //               {
  ...
  //             ticks: {
  //               color: "#8b949e",
  //               stepSize: 1, // Ensure we show every member
  //               callback: function (value) {
  //                 // Only show member names at exact integer positions
  //                 const memberIndex = Math.round(value);
  //                 return memberIndex >= 0 && memberIndex < members.length ? members[memberIndex] : "";
  //               },
  //             },
  //             min: -0.5,
  ...
  //             title: {
  //               display: true,
  //               text: `Working Hours (${config.start_hour}:00 - ${config.end_hour}:00)`,
  //               color: "#f0f6fc",
  //             },
  //             grid: {
  //               color: "rgba(255, 255, 255, 0.1)",
  ...
  //                 // Only show member names at integer positions
  //                 const memberIndex = Math.floor(value);
  //                 if (memberIndex >= 0 && memberIndex < members.length && value === memberIndex) {
