...
API_TOKEN = 'pk_99989505_RJXIV1Q62WJZDW5P5J30JDYRGHVF7N6R' # Tricia
...
def calculate_downtime_today(member_name, in_progress_periods, today_start, current_time):
    ...
    excluded_recipients = {"Sean", "Rick", "Tricia", "Jan"}
    ...
    # === Send email via ClickUp or external system if needed ===
    if notify_needed and member_name not in excluded_recipients:
        send_downtime_email(member_name, downtime_periods)

    return downtime_periods

def send_downtime_email(member_name, downtime_periods):
    print(f"      [EMAIL] Sending downtime alert to {member_name} via ClickUp or external service...")
